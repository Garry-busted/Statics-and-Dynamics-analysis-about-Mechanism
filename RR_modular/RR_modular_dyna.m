...
for i = 1:length(t2)
    [L4(i),L3(i),h4(i),h3(i),Q(i),P(i),tQ(i),tP(i),ag4x(i),ag4y(i),ag3x(i),ag3y(i)] = preForceRR(r4,t4(i),t3(i),phi4,phi3,m4,m3,b4,b3,I4,I3,t4d(i),t3d(i),t4dd(i),t3dd(i));
    [F34_m(i),F34t_m(i),F34n_m(i),Fex_m(i),Fey_m(i),F14x_m(i),F14y_m(i)] = ForceRR(P(i),Q(i),0,0,r4,r3,L4(i),L3(i),h4(i),h3(i),t4(i),t3(i),tP(i),tQ(i),m4,m3,b4,b3,phi4,phi3,G);
    ...
    ag2(i) = sqrt(ag2x_current^2 + ag2y_current^2);
    % 計算 M12
    if G == 0
    ...
    + b2*sin(t2(i)+phi2)*m2*ag2x_current - b2*cos(t2(i)+phi2)*m2*ag2y_current;
        ...
        M12_m(i) = -(r2*cos(t2(i))) * Fey_m(i) + (r2*sin(t2(i))) * Fex_m(i) ...
    + b2*sin(t2(i)+phi2)*m2*ag2x_current - b2*cos(t2(i)+phi2)*m2*ag2y_current -m2*g*b2*cos(t2(i)+phi2);
    end
    ...
    if G == 0
        ...
        F12y_m(i) = Fey_m(i) - m2 * ag2y_current + m2*g;
    end
    
    % 4. 計算搖撼力與力矩
    % 1202：不考慮重力下，兩種定義下的搖撼力算出來都一樣
    Fs_m(i) = sqrt((F12x_m(i) + F14x_m(i))^2 + (F12y_m(i) + F14y_m(i))^2);
    Fs(i) = sqrt((m2*ag2x_current + m3*ag3x(i) + m4*ag4x(i))^2 + (m2*ag2y_current + m3*ag3y(i) + m4*ag4y(i))^2);
...
hold on;
grid on;
plot(theta2, Fs_m, '-b', 'LineWidth', 1.5);
plot(theta2, Fs, '-r', 'LineWidth', 1.5);
xlim([0,360]);
xticks(0:60:360);
hold off;
title('不同定義的搖撼力');
