...
hold on;
grid on;
plot(theta2, Fs,'-b','LineWidth',3);
plot(theta2, Fs_m, 'r-','LineWidth',2);
xlim([0,360]);
xticks(0:60:360);
hold off;
title('搖撼力 (F_s)');
...
hold on;
grid on;
plot(theta2, Ms,'-b','LineWidth',3);
plot(theta2, Ms_m, 'r-','LineWidth',2);
xlim([0,360]);
xticks(0:60:360);
hold off;
title('搖撼力矩 (M_s)');
...
hold on;
grid on;
plot(theta2, M12,'-b','LineWidth',3);
plot(theta2, M12_m, '-r','LineWidth',2);
xlim([0,360]);
xticks(0:60:360);
hold off;
title('驅動力矩 (M_{12})');
xlabel('輸入搖桿角度 \theta_2 (度)');
ylabel('M_{12} (N\cdot m)');
legend('M12', 'M12_m');

subplot(3,2,4);
hold on;
grid on;
plot(theta2, F32,'-b','LineWidth',3);
plot(theta2, Fe_m, '-r','LineWidth',2);
xlim([0,360]);
xticks(0:60:360);
hold off;
title('旋轉接頭受力 (F_{32})');
...
hold on;
grid on;
alphas_conti = unwrap(alphas);
alphas_m_conti = unwrap(alphas_m);
plot(theta2, alphas_conti, '-b');
plot(theta2, alphas_m_conti, '--r');
xlim([0,360]);
xticks(0:60:360);
hold off;
title('搖撼力夾角 (\alpha_s)');
